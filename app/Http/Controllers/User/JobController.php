...
class JobController extends Controller
{
    ...
    { 
        $page = 'job';
        $totalCount = Job::where('user_id', Auth::user()->id)->count();
        $waitCount = Job::where('user_id', Auth::user()->id)->where('status', 'waiting')->count();
        $approvedCount = Job::where('user_id', Auth::user()->id)->where('status', 'approved')->count();
        $rejectCount = Job::where('user_id', Auth::user()->id)->where('status', 'reject')->count();
        ...
        if  ($request->has('jobSearch')) {
                ...
                $data = Job::where('user_id', Auth::user()->id)->where('title', 'LIKE', "%{$searchTerm}%")
                        ->paginate(15);
                return view('Pages.User.Job.Job', compact('page', 'data', 'dates', 'searchTerm', 'currentdate', 'totalCount', 'waitCount', 'approvedCount', 'rejectCount'));    
                ...
                return view('Pages.User.Job.Job', compact('page', 'data', 'dates', 'currentdate', 'totalCount', 'waitCount', 'approvedCount', 'rejectCount'));
        }
    }
    ...
    {
        $page = 'job';
        return view('Pages.User.Job.AddJob', compact('page'));
    }
    ...
    public function viewjob($id)
    {
        ...
        return view('Pages.User.Job.ViewJob', compact('page', 'data'));
    }
    ...
    public function jobedit($id)
    {
        $page = 'job';
        $data = Job::where('id', $id)->first();
        return view('Pages.User.Job.EditJob', compact('page', 'data'));
    }
    ...
    public function submitjob(Request $request)
    {
        ...
        return redirect()->route('user.job')->with('message', 'Job Inserted Successfully');
    }
    ...
    public function updatejob(Request $request)
    {
        ...
        return redirect()->route('user.job')->with('message', 'Job Updated Successfully');
    }
