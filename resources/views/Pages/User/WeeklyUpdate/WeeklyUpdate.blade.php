    ...
    <div class="px-10 bg-white py-5 rounded-3 shadow-lg dark:bg-slate-850 dark:shadow-dark-xl">
        <div class="flex justify-between">
            ...
            <div>
                ...
                <a href="{{ route('user.addweeklyupdate') }}" class="bg-blue-500 px-5 py-2 text-[14px] text-white rounded-2 cursor-pointer">
                    Add Weekly Update
                </a>
            </div>
        </div>
                        ...
                        <tbody class="updatedData">
                            @foreach ($data as $item)
                                <tr class="work">
                                    ...
                                    <td
                                        class="p-2 align-middle bg-transparent border-b dark:border-white/40 whitespace-nowrap shadow-transparent">
                                        <a href="{{ route('user.weekUpdateView', $item->id) }}" class="py-1 px-2 bg-blue-500 rounded-1 text-white">
...
@section('JS')
    ...
    <script>
        ...
        const dateHandler = (date) => {
            ...
            if (value == 0) {
                window.location.href = `{{ url('user/weeklyupdate') }}`;
            }
            else{
                window.location.href = `{{ url('user/weeklyupdate') }}/${value}`;
            }
        }
    </script>
@endsection
