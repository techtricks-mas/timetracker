...
@section('JS')
    ...
    <script>
        ...
        const calanderHandler = (eventsArray) => {
          ...
          var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventContent: function(info) {
                ...
                const s_date = event.start;
                const sdate = new Date(s_date);
                let createdAtEl = document.createElement('p');
                createdAtEl.classList.add('start');
                createdAtEl.innerText = 'Start date: ' + sdate.getFullYear() + '-' + (sdate.getMonth() + 1).toString().padStart(2, '0') + '-' + sdate.getDate().toString().padStart(2, '0');

                const e_date = event.end;
                const edate = new Date(e_date);
                let dateEl = document.createElement('p');
                dateEl.classList.add('date');
                dateEl.innerText = 'End date: ' + edate.getFullYear() + '-' + (edate.getMonth() + 1).toString().padStart(2, '0') + '-' + edate.getDate().toString().padStart(2, '0');
                ...
                extraInfoEl.appendChild(dateEl);
        ...
        $.ajax({
            type: "Get",
            url: "{{ route('candidate.getHoliday') }}",
            data: "data",
    ...
    <script>
        ...
        const submitHandler = () => {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('candidate.addholiday') }}",
                data: {
                    datestart: dateStart,
                    dateend: dateEnd,
                    event: event,
        ...
        const updateHandler = () => {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('candidate.updateholiday') }}",
                data: {
                    edit: edit,
                    datestart: dateStart,
                    dateend: dateEnd,
        ...
        const deleteHandler = () => {
            ...
            $.ajax({
                type: "post",
                url: "{{ route('candidate.deleteholiday') }}",
                data: {
                    edit: edit,
                    _token: token
                },
