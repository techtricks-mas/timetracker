...
@section('content')
    <div class="px-10 bg-white py-5 rounded-3 shadow-lg dark:bg-slate-850 dark:shadow-dark-xl">
        ...
        <div>
            <form method="POST" action="{{ route('candidate.updatecandidatetinterview') }}" name="form">
                ...
                <div class="md:flex py-3">
                    <div class="w-full md:w-1/2 md:mr-2">
                        ...
                        <input id="role" type="text"  value="{{ $data->role }}"
                            class="@error('role') border-red-500 @enderror dark:bg-slate-850 dark:border-white dark:text-white dark:bg-slate-850 dark:border-white dark:text-white px-3 py-2 rounded-2 border border-black focus:outline-none w-full"
                            name="role" />
                        @error('role')
                            <p class="alert alert-danger text-red-500 text-sm">{{ $message }}</p>
                        @enderror
